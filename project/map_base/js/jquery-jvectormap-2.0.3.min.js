...
jvm.$ = jQuery, Array.prototype.indexOf || (Array.prototype.indexOf = function(searchElement, fromIndex) {
	...
	var that = this;
	"image" == attr ? jvm.whenImageLoaded(value).then(function(img) { //原来的that.width = img[0].width, that.height = img[0].height
		that.node.setAttributeNS("http://www.w3.org/1999/xlink", "href", value), that.width = 40, that.height = 40, that.applyAttr("width", that.width), that.applyAttr("height", that.height), that.applyAttr("x", that.cx - that.width / 2), that.applyAttr("y", that.cy - that.height / 2), jvm.$(that.node).trigger("imageloaded", [img])
